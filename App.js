...
async function fetchForecast(city) {
  try {
    ...
    forecastContainer.innerHTML = ""; //removes old data befor adding new data 
    ...
    forecastData.list.forEach(item => {
      ...
      if (!dailyForecasts[forecastDate] && time === "12:00:00") {
        dailyForecasts[forecastDate] = item;
                            ...
                            // convert to array object value method
    const filteredForecasts = Object.values(dailyForecasts).slice(0, 5);

    filteredForecasts.forEach(item => {
      const forecastDate = new Date(item.dt * 1000);// converts unix data to date object
      const options = { weekday: 'short', month: 'short', day: 'numeric' };
      const dayString = forecastDate.toLocaleDateString(undefined, options);//formating 
      const temp = Math.round(item.main.temp) + "°C"; // it roundfiger
      ...
      forecastItem.innerHTML = `
        ...
        <p>${temp}</p>
      `;
      forecastContainer.appendChild(forecastItem);// join items
    });

    forecastSection.style.display = "block";
  } catch (error) {
    console.error("Error fetching forecast data:", error);// if erroe forcastsections is not visible
    forecastSection.style.display = "none";
  }
}
...
function applyWeatherAnimation(weatherCondition) {
    ...
    body.classList.remove("rainy", "snowy", "clear", "cloudy","haze","smoke");
